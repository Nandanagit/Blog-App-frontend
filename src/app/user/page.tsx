...
function UserProfilePage() {
  ...
  useEffect(() => {
    ...
    const username = localStorage.getItem('name');
      if (!username) return;
      try {
        const res = await fetch(`https://nandana-blog-backend.vercel.app/auth/profile`, {
          headers: {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
     const username = localStorage.getItem('name');
      if (!username) return;
      try {
        const res = await fetch(`https://nandana-blog-backend.vercel.app/posts/author/${username}`, {
           headers: {
            'Authorization': 'Bearer ' + localStorage.getItem('jwt_token'),
            'Content-Type': 'application/json',
          },
  ...
  const handleBlogSave = async (id: number) => {
    ...
    if (!username) return;
    try {
      const res = await fetch(`https://nandana-blog-backend.vercel.app/posts/update/${id}`, {
        method: 'POST',
        headers: {
          'Authorization': 'Bearer ' + localStorage.getItem('jwt_token'),
          'Content-Type': 'application/json',
                ...
                {editingBlogId !== blog.id ? (
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      onClick={async () => {
                        try {
                          await fetch(`https://nandana-blog-backend.vercel.app/posts/delete/${blog.id}`, {
                            method: 'DELETE',
                            headers: {
                              'Authorization': 'Bearer ' + localStorage.getItem('jwt_token'),
                              'Content-Type': 'application/json',
